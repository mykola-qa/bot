...
class MongoDBClient:
    ...
    async def save_message(self, user_id: str, message: str, assistant: bool) -> None:
        ...
        await collection.insert_one({'user_id': user_id, 'message': message, 'assistant': assistant})
        logging.info(f'Message {user_id}{message[:10]}... is saved to db. ')

    async def get_context_by_user_id(self, user_id: str) -> list:
        db = self.client[self.database_name]
        collection = db[self.collection_messages]
        async_cursor = collection.find({'user_id': user_id}).sort(key_or_list='_id', direction=pymongo.DESCENDING)
        counter = 30
        messages = []
        async for entry in async_cursor.sort(key_or_list='_id', direction=pymongo.DESCENDING):
            del entry['_id']
